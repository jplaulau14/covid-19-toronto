...
def load_local(df: pd.DataFrame) -> Path:
    ...
    """
    df.to_parquet(f"data/{current_datetime}-covid19-toronto.parquet")
    return Path(f"data/{current_datetime}-covid19-toronto.parquet")
...
def extract_from_gcs() -> Path:
    ...
    """
    gcs_path = f"data/{current_datetime}-covid19-toronto.parquet"
    gcs_block = GcsBucket.load('gcs-block')
...
@task(retries=3, retry_delay_seconds=10, log_prints=True, name="Run dbt")
def run_dbt():
    """
    Run dbt models
    """
    dbt_path = "project_dbt"
